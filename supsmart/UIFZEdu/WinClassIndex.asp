...
<%if MO_Win>=1 then%>
	...
	<html><head>
			<meta charset="utf-8">
			<title>优典集团丰正校区管理系统-教师排课桌面看板</title>
	...
	%>
		...
		<form action="WinClassIndex.asp"  name="form1" method="post"  target="_self" >
	...
	<table border="1">
		...
		<tr><td style="width: 10px;" id="T<%=TeaName(1+i,1)%>"><%=TeaName(1+i,2)%>
			...
			if (now()-30)> (cdate(session("Bday"))+ii-1) then
				DayLock=1
				CheckOpenLock=""
			end if
			...
			<td>
				<table border="0">
					...
					<tr>
						...
						for hi=7 to 21
							ToWinInf0=TeaName(1+i,1)&"_"&TeaName(1+i,2)&"_"&FormatDate(cdate(session("Bday"))+ii-1,2)&"_"&hi&"_"&TeaName(1+i,3)
							ToWinInf1=""
							if SqlR(0,3)=0 then
										...
										<tr  border="1" style="background-color: <%=TCtypeColor%>;">
											<TD	 rowspan=<%=ChourNum%>  style="display: none;" id="Del<%=SqlR(C_i,1)%>">
												<%if SqlR(C_i,18)=1 and MO_Win<9 then%>
													<img src="../images/lock.png" />	
												<%else%>
													<DIV class="bo bo1" onclick="DelClass(this);" id="<%=SqlR(C_i,1)%>,<%=TeaName(1+i,1)%>">作废此课</DIV>
												<%end if%>
